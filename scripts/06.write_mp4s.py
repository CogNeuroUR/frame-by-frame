...
i=0
for index, row in mifs_positioning.iterrows(): # AB: Does this loop iterate e.g. over all our MIT-vodeos? if yes please satet her additionaly
  i+=1
  
  if i % 50 == 0:
    print(f'{i}/{len(mifs_positioning)}')
  if i < len(mifs_positioning) + 1000:  # "+1000" to be sure :-P
    ...
    # Check if frames_list has negative entries and if so, exlude and report
    if frames_list[0] < 0:
