...
path_to_features = Path(f'../temp/outputs/RN50_features_softmax_average.csv')
path_to_rdm = Path(f'../temp/outputs/rdm_average_softmax_cosine.csv')

df_features = pd.read_csv(path_to_features)
df_rdm = pd.read_csv(path_to_rdm)

features = df_features.values[:,1:340]
...
sns.heatmap(rdm_original[:, 306:], ax=ax, cmap="GnBu", #linewidths=0.01,
...
#plt.savefig(f'../plots/silhouette_features_n221_ward.pdf')
plt.show()
...
plt.tight_layout()
#plt.savefig(f'../plots/dendrogram_features_n221_ward.pdf')
